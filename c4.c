...
char *p = NULL, *lp = NULL,       // current position in source code
                    *data = NULL; // data/bss pointer

int *e = NULL, *le = NULL,          // current position in emitted code
                   *id = NULL,      // currently parsed identifier
                       *sym = NULL, // symbol table (simple list of identifiers)
    tk = 0,                         // current token
    ...
    debug = 0;                      // print executed instructions
...
enum
{
    ...
    Glo,    // 131
    Loc,    // 132
    Id,     // 133 identifier 用户定义的identifier
    Char,   // 134
    Else,   // 135
    Enum,   // 136
    If,     // 137
    ...
    while (tk = *p)
    {
        ...
        else if ((tk >= 'a' && tk <= 'z') || (tk >= 'A' && tk <= 'Z') || tk == '_')
        {
            ...
            while ((*p >= 'a' && *p <= 'z') || (*p >= 'A' && *p <= 'Z') || (*p >= '0' && *p <= '9') || *p == '_')
                tk = tk * 147 + *p++;  // 计算哈希值,tk起始时已经等于*p,从p开始正确 147=1001 0011
            tk = (tk << 6) + (p - pp); // 哈希值加上[符号长度] ,6位bit最大63
            id = sym;
            while (id[Tk])
            {
                ...
                id = id + Idsz; // 继续循环identifier表 Idsz=9
            }
        ...
        { // 引号开头,认为是字符(串)
            ...
            if (tk == '"')
                ...
                tk = Num; // 双引号则ival指向data中字符串开始,单引号则认为是数字
            return;
        }
        else if (tk == '=')
        {
...
int vm_execute(int *pc, int *sp, int argc, char **argv)
{
    int *bp, ax; // vm register
    int *t;      // temp var
    int cycle = 0;
    int i = 0;
    // setup stack
    ...
    *--sp = (int)t;
    ...
    while (1)
    {
        ...
        if (i == LEA)
            ax = (int)(bp + *pc++); // load local address
        else if (i == IMM)
            ax = *pc++; // load global address or immediate
        else if (i == JMP)
            pc = (int *)*pc; // jump
        else if (i == JSR)
        {
            *--sp = (int)(pc + 1);
            ...
            pc = ax ? pc + 1 : (int *)*pc; // branch if zero
        else if (i == BNZ)
            pc = ax ? (int *)*pc : pc + 1; // branch if not zero
        else if (i == ENT)
        {
            *--sp = (int)bp;
            bp = sp;
        ...
        {
            t = sp + pc[1];
            ax = printf((char *)t[-1], t[-2], t[-3], t[-4], t[-5], t[-6]);
        }
        else if (i == MALC)
            ax = (int)malloc(*sp);
        else if (i == MSET)
...
int main(int argc, char **argv)
{
    ...
    // bt basetype
    int fd, bt, ty, *idmain;
    int *pc, *sp; // vm registers 虚拟器的寄存器
    int i;        // temps
    ...
    return vm_execute(pc, sp, argc, argv);
}
